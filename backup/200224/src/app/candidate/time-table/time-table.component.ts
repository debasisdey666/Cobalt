...
export class TimeTableComponent implements OnInit {
  ...
  transformDataForFullCalendar(apiData: any[]): any[] {
    return apiData.map((item: any) => ({
      ...
      extendedProps: {
        ...
        INSTRUCTOR_NAME: item.INSTRUCTOR_NAME, // Store END_TIME as an extended property
        ROOM_NO: item.ROOM_NO, // Store END_TIME as an extended property
      },
  ...
  handleDateClick(arg: any) {
    ...
    const instructor = arg.event.extendedProps.INSTRUCTOR_NAME;
    const ROOM_NO = arg.event.extendedProps.ROOM_NO;

    if (ROOM_NO.length > 0) {
      alert(`Class Name : ${title} \nClass Time : ${starttime} - ${end} \nFaculty Name : ${instructor} \nRoom No. : ${ROOM_NO}`);
    } else {
        alert(`Class Name : ${title} \nClass Time : ${starttime} - ${end} \nFaculty Name : ${instructor}`);
    }
