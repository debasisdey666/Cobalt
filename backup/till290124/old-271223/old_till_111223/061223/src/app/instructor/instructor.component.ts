  ...
  DateString(inputdate: Date | undefined): string | undefined {
    ...
    editInstructor(formData: any) {
      ...
      // const parsedFormattedPapers = JSON.parse(formattedPapersString);
       ...
       // Get the existing selected papers
        const existingSelectedPapers = this.showpr.filter((papDropdwn) => papDropdwn.isChecked && papDropdwn.ID);

        // Get the newly selected papers
        const newSelectedPapers = this.showpr.filter((papDropdwn) => papDropdwn.isChecked && !papDropdwn.ID);

        // Merge the existing and new selected papers
        const mergedSelectedPapers = [...existingSelectedPapers, ...newSelectedPapers];

        // Format the merged selected papers
        const formattedPapers = mergedSelectedPapers.map((papDropdwn) => ({
          paper_ID: papDropdwn.ID.toString(),
        }));
        console.log('Formatted Papers:', formattedPapers);
