...
export class PaperService {
  ...
  showPaperDropdownBindChapter(selectedSemesterId: number,selectedBranchId:number,selectedACId:number): Observable<any> {
    const userId = getFromLocalStorage('userId');
    const data = {
      "useR_ID": userId,
      "dropdowN_TYPE": 2,
  ...
  showPaperDropdownBindEditChapter(SEMID:number,BRANCHID:number,AYID:number): Observable<any> {
    const userId = getFromLocalStorage('userId');
    const data = {
      "useR_ID": userId,
      "dropdowN_TYPE": 2,
      "aY_ID": AYID,
      "brancH_ID": BRANCHID,
      "seM_ID": SEMID // Pass the selected semester ID
    };
    return this.http.post(this.url2, data);
  }
